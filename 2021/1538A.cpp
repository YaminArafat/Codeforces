...
int main()
{
    ...
    {
        cin >> n;
        int arr[n + 5], maxx = INT_MIN, minn = INT_MAX, maxPos = 0, minPos = 0, ans = 0;
        for0(i, n, 1)
        {
            cin >> arr[i];
        }
        ...
        if (maxPos > minPos)
        {
            ans = min(n - minPos + 1, min(minPos + n - maxPos + 1, maxPos));
        }
        else
        {
            ans = min(n - maxPos + 1, min(maxPos + n - minPos + 1, minPos));
        }
        cout << ans << endl;
    }
    return 0;
